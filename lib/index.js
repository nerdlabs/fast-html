 ...
 */
module.exports = function parse(data, createElementNode, createTextNode, parseAttrs) {
    ...
    while ((match = pattern.markup.exec(data)) !== null) {
        var tag                        = match[0],
            tagName                    = match[2],
        ...
        isSelfClosingTag = isExplicitlySelfClosingTag || isImplicitlySelfClosingTag;
        ...
        if (!isClosingTag) {
            ...
            if (tagName === 'script') {
                ...
                closeMarkup = '</script>';
                index = data.indexOf(closeMarkup, pattern.markup.lastIndex);
